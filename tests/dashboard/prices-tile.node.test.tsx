import { test } from 'node:test';
import assert from 'node:assert/strict';
import React from 'react';
import { renderToString } from 'react-dom/server';
import { PricesClient } from '../../components/dashboard/tiles/CurrentPricesTile';
import CurrentPricesTile from '../../components/dashboard/tiles/CurrentPricesTile';

/**
 * Ensure the prices tile renders a table row for each provided quote.
 */
test('renders provided quotes', () => {
  const html = renderToString(
    <PricesClient
      initialQuotes={[
        {
          symbol: 'AAPL',
          price: 123.45,
          change: 0.5,
          changePercent: 0.4,
          marketState: 'REG',
        },
      ]}
    />,
  );
  assert.match(html, /AAPL/);
  assert.match(html, /123\.45/);
});

/**
 * Empty quote list should render placeholder text.
 */
test('renders empty state', () => {
  const html = renderToString(<PricesClient initialQuotes={[]} />);
  assert.match(html, /Aucun cours à afficher/);
});

/**
 * The server wrapper should gracefully handle fetch errors and still render the
 * empty state so the rest of the dashboard is not affected.
 */
test('CurrentPricesTile handles fetch errors', async () => {
  const originalFetch = global.fetch;
  const originalError = console.error;
  // @ts-ignore override fetch to simulate network failure
  global.fetch = async () => {
    throw new Error('network failure');
  };
  console.error = () => {};
  const html = renderToString(await CurrentPricesTile());
  assert.match(html, /Aucun cours à afficher/);
  global.fetch = originalFetch;
  console.error = originalError;
});
